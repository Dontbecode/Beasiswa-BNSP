    ...
    if (!empty($validationErrors)) {
        ...
        if (!preg_match('/\./', $domain)) {
            ...
            if (!in_array($topLevelDomain, $allowedTopLevelDomains)) {
                ...
                $targetDirectory = "../../Doc/"; // Direktori untuk menyimpan berkas
                $targetPath = $targetDirectory . basename($namaBerkas);
                $uploadOk = 1;
                ...
                if ($uploadOk == 1) {
                    // Lakukan upload file ke folder Doc/
                    if (move_uploaded_file($_FILES["berkas_syarat"]["tmp_name"], $targetPath)) {
                        // Lakukan penyimpanan ke database
                        // Sertakan koneksi ke database
                        require_once '../../Connect.php';
                        $statusAjuan = "Belum Terverifikasi";
                        $sql = "INSERT INTO tabel_pendaftaran(id, nama_lengkap, email, nomor_hp, semester, ipk, pilihan_beasiswa, berkas_syarat, status_ajuan) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                        $stmt = mysqli_prepare($koneksi, $sql);
                        mysqli_stmt_bind_param($stmt, 'sssssssss', $uuid, $nama_lengkap, $email, $nomor_hp, $semester, $ipk, $pilihan_beasiswa, $namaBerkas, $statusAjuan);
