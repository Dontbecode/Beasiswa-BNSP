...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Simpan ke database
    require_once '../../Connect.php'; // Sertakan file koneksi
    // Cek apakah nama sudah ada dalam database
    $check_query = "SELECT id FROM tabel_beasiswa WHERE nama_beasiswa = ?";
    $check_stmt = mysqli_prepare($koneksi, $check_query);
    mysqli_stmt_bind_param($check_stmt, "s", $nama_beasiswa);
    ...
    if (!empty($validationErrors)) {
        ...
        $sql = "INSERT INTO tabel_beasiswa (id, nama_beasiswa) VALUES (?, ?)";
        $stmt = mysqli_prepare($koneksi, $sql);
        mysqli_stmt_bind_param($stmt, "ss", $uuid, $nama_beasiswa);
